...
static __global__ void _thinZSPt1Ker(ImageCuda tempimg, ImageCuda outimg, int *devchangecount)
{
    // dstc 和 dstr 分别表示线程处理的像素点的坐标的 x 和 y 分量 （其中，c 表示
    // column，r 表示 row ）。
    int dstc = blockIdx.x * blockDim.x + threadIdx.x;
...
static __global__ void _thinZSPt2Ker(ImageCuda tempimg, ImageCuda outimg, int *devchangecount)
{
    // dstc 和 dstr 分别表示线程处理的像素点的坐标的 x 和 y 分量 （其中，c 表示
    // column，r 表示 row ）。
    int dstc = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // 局部变量，错误码。
     ...
     while (changeCount > 0) {
        // iter_num ++;
         ...
         // 调用核函数，开始第一步细化操作。
         _thinZSPt1Ker<<<gridsize, blocksize>>>(tempsubimgCud, outsubimgCud, devchangecount);
         if (cudaGetLastError() == cudaSuccess) {
             i++;
         }
         ...
         // 调用核函数，开始第二步细化操作。
         _thinZSPt2Ker<<<gridsize, blocksize>>>(outsubimgCud, tempsubimgCud, devchangecount);
         if (cudaGetLastError() == cudaSuccess) {
             i++;
         }
