...
int main(int argc, char const **argv)
{
    ...
    for (int dev = 0; dev < deviceCount; ++dev) {
        ...
        for (int by = 0; by <= 32; by += 2)
        {
            ...
            ImageBasicOp::writeToFile("thinZS_outimg.bmp", outimg1); 
            ...
            cudaEventRecord(start, 0);
            for (int i = 0; i < LOOP; i++) 
                thin_gpu.thinZSPt(inimg, outimg2);
            cudaEventRecord(stop, 0);
            cudaEventSynchronize(stop);
            cudaEventElapsedTime(&runTime, start, stop);
            cout << "thinZSPt() time is " << (runTime) / LOOP << " ms" << endl;
            ImageBasicOp::copyToHost(outimg2);
            ImageBasicOp::writeToFile("thinZSPt_outimg.bmp", outimg2); 

            // cudaEventCreate(&start);
            // cudaEventCreate(&stop);
            // cudaEventRecord(start, 0);
            // for (int i = 0; i < LOOP; i++) 
            //     thin.thinZSFour(inimg, outimg3);
            // cudaEventRecord(stop, 0);
            // cudaEventSynchronize(stop);
            // cudaEventElapsedTime(&runTime, start, stop);
            // cout << "thinZSFour() time is " << (runTime) / LOOP << " ms" << endl;
            // ImageBasicOp::copyToHost(outimg3);
            // ImageBasicOp::writeToFile("thinZSFour_outimg.bmp", outimg3); 
            ...
            // cudaEventRecord(start, 0);
            // for (int i = 0; i < LOOP; i++) 
            //     thin.thinZSPtFour(inimg, outimg4);
            // cudaEventRecord(stop, 0);
            // cudaEventSynchronize(stop);
            // cudaEventElapsedTime(&runTime, start, stop);
            // cout << "thinZSPtFour() time is " << (runTime) / LOOP << " ms" << endl;
            // ImageBasicOp::copyToHost(outimg4);
            // ImageBasicOp::writeToFile("thinZSPtFour_outimg.bmp", outimg4); 
