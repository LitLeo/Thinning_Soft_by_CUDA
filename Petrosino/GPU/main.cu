    ...
    for (int dev = 0; dev < deviceCount; ++dev) {
        ...
        for (int by = 0; by <= 32; by += 2)
        {
            ...
            for (int i = 0; i < LOOP; i++) 
                thin_gpu.thinPet(inimg, outimg1);
            cudaEventRecord(stop, 0);
            cudaEventSynchronize(stop);
            cudaEventElapsedTime(&runTime, start, stop);
            cout << "thinPet() time is " << (runTime) / LOOP << " ms" << endl;
            ImageBasicOp::copyToHost(outimg1);
            ImageBasicOp::writeToFile("thinPet_outimg.bmp", outimg1); 
            ...
            cudaEventRecord(start, 0);
            for (int i = 0; i < LOOP; i++) 
                thin_gpu.thinPetPt(inimg, outimg2);
            cudaEventRecord(stop, 0);
            cudaEventSynchronize(stop);
            cudaEventElapsedTime(&runTime, start, stop);
            cout << "thinPetPt() time is " << (runTime) / LOOP << " ms" << endl;
            ImageBasicOp::copyToHost(outimg2);
            ImageBasicOp::writeToFile("thinPetPt_outimg.bmp", outimg2); 
            ...
            // cudaEventRecord(start, 0);
            // for (int i = 0; i < LOOP; i++) 
            //     thin_gpu.thinPetFour(inimg, outimg4);
            // cudaEventRecord(stop, 0);
            // cudaEventSynchronize(stop);
            // cudaEventElapsedTime(&runTime, start, stop);
            // cout << "thinPetFour() time is " << (runTime) / LOOP << " ms" << endl;
            // ImageBasicOp::copyToHost(outimg4);
            // ImageBasicOp::writeToFile("thinPetFour_outimg.bmp", outimg4); 
            ...
            // cudaEventRecord(start, 0);
            // for (int i = 0; i < LOOP; i++) 
            //     thin_gpu.thinPetPtFour(inimg, outimg5);
            // cudaEventRecord(stop, 0);
            // cudaEventSynchronize(stop);
            // cudaEventElapsedTime(&runTime, start, stop);
            // cout << "thinPetPtFour() time is " << (runTime) / LOOP << " ms" << endl;
            // ImageBasicOp::copyToHost(outimg5);
            // ImageBasicOp::writeToFile("thinPetPtFour_outimg.bmp", outimg5); 
